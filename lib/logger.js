...
function browserLog(theme, time, value) {
  if (enableColors) {
    console.log(
    ...
    console.log(`${time} [${theme.label}]`, value);
  }
}

function serverLog(theme, time, value) {
...
function handleLog(level, value) {
  const theme = LOG_THEMES[level];
  const time = `[${new Date().toLocaleTimeString()}]`;

  if (isBrowser) {
    browserLog(theme, time, value);
  } else {
    serverLog(theme, time, value);
  }
}

export const logger = {
  info: (value) => handleLog("INFO", value),
  success: (value) => handleLog("SUCCESS", value),
  warn: (value) => handleLog("WARN", value),
  error: (value) => handleLog("ERROR", value),
};
